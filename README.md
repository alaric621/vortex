...
### 3.1.1 变量装饰器刷新说明
- **自动刷新**：在 `.vht` 文档中编辑文本、移动光标、切换到其他编辑器时，变量装饰器会自动刷新。
- **编辑优先**：当光标进入某个变量占位符（如 `{{name}}`）时，会恢复原始变量文本，便于直接修改；光标移开后恢复显示替换值。
- **手动刷新**：
  - 先切换到其他文件再切回当前 `.vht` 文件；
  - 或执行 `Developer: Reload Window` 强制刷新扩展状态。

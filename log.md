...
# Role
你现在是一位资深的软件架构师和代码审计专家。

# Task
请先深入理解我的项目结构和核心逻辑，暂时不要开始编写代码。

# Project Context
- **技术栈:** vscode api
- **核心功能:** [类似于 Postman 但针对国内习惯做了大量优化。

多协议支持： 支持 HTTP/HTTPS、WebSocket、gRPC、CoAP 等。

可视化参数填写： 无需手动写 JSON，支持通过表单直接生成 Query、Body 参数。

实时调试： 发送请求后实时查看 Response、Header、Cookie 以及耗时分析。

预执行/后执行脚本： 支持 JS 脚本进行断言、变量提取（如登录后自动将 Token 存入全局变]
- **项目结构:** [在此处粘贴 `tree` 命令生成的目录树，或描述主要文件夹结构]
